...
void childProcess(int childNum, int read_fd, int write_fd, int sortedList[], int count) {
    ...
    char receiveToken[TOKEN_SIZE];

    strcpy(token, "GO_AHEAD");

    read(read_fd, receiveToken, sizeof(receiveToken)); // Read token from parent

    if (strcmp(receiveToken, token) == 0) {
      ...
      // Pass token to the next child
        ...
        write(write_fd, receiveToken, sizeof(receiveToken));
    }
